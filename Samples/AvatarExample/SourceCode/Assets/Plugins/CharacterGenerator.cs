...
public class CharacterGenerator
{
    ...
    //Defines the key for character during searches
    public static readonly string CHARACTER_PROP = "character";

    //A list of synonyms a user can say to refer to a property of their avatar
    static Dictionary<string, string[]> wordsForCategory;

    //A list of possible adjectives that can map to properties that are specific to the character
    static Dictionary<string, Dictionary<string, Dictionary<string, string>>> propertyDescriptors;
	...
	public static bool ReadyToUse
	{
		get
		{
            ...
            //Set up propertyDescriptor
            propertyDescriptors = new Dictionary<string, Dictionary<string, Dictionary<string, string>>>();
			...
			foreach (CharacterElement element in ceh.content)
			{
                ...
                string[] b = element.name.Split('_');
                string descriptor = (b.Length > 2) ? b[2].Split('-')[0] : null;
                ...
                if (!propertyDescriptors.ContainsKey(character))
                    propertyDescriptors.Add(character, new Dictionary<string, Dictionary<string, string>>());

                if (!propertyDescriptors[character].ContainsKey(category))
                {
                    propertyDescriptors[character].Add(category, new Dictionary<string, string>());
                    propertyDescriptors[character][category]["next"] = "next";
                    propertyDescriptors[character][category]["previous"] = "prev";
                }
                if (descriptor != null)
                    propertyDescriptors[character][category][descriptor] = element.name;

			}

            //Set up wordsForCategory
            string[] wordsForCharacter = { "character", "person", "avatar" };
            string[] wordsForFace = { "face", "head" };
            string[] wordsForEyes = { "eye", "pupil", "ice" };
            string[] wordsForHair = { "hair", "dew", "air", "due" };
            string[] wordsForTop = { "shirt", "top", "sweater", "jacket", "body" };
            string[] wordsForPants = { "pant", "leg", "bottom", "jeans" };
            string[] wordsForShoes = { "shoe", "feet", "foot", "boot", "sandle" };

            wordsForCategory = new Dictionary<string, string[]>();
            wordsForCategory.Add("character", wordsForCharacter);
            wordsForCategory.Add("face", wordsForFace);
            wordsForCategory.Add("eyes", wordsForEyes);
            wordsForCategory.Add("hair", wordsForHair);
            wordsForCategory.Add("top", wordsForTop);
            wordsForCategory.Add("pants", wordsForPants);
            wordsForCategory.Add("shoes", wordsForShoes);
			return true;
		}
	}
    ...
    private static int LastIndexOfStringInText(string textToCheck, string[] possibleStrings)
    {
        int maxIndex = -1;
        foreach (string subString in possibleStrings)
            maxIndex = Math.Max(maxIndex, textToCheck.LastIndexOf(subString));

        return maxIndex;
    }

    public Dictionary<string, string> ParseForProperties(string text)
    {
        Dictionary<string, string> results = new Dictionary<string, string>();
        string defaultResult = "next";

        foreach (KeyValuePair<string, string[]> wordCategory in wordsForCategory)
        {
            int lastIndexForCategory = LastIndexOfStringInText(text, wordCategory.Value);
            if (lastIndexForCategory > 0)
            {
                int endOfPrevWord = text.LastIndexOf(' ', lastIndexForCategory);
                int startOfPrevWord = text.LastIndexOf(' ', endOfPrevWord - 2);
                string newCategoryValue = defaultResult;

                Debug.Log("lastIndexForCategory = " + lastIndexForCategory + ", startOfPrevWord = " + startOfPrevWord);

                if (!wordCategory.Key.Equals(CHARACTER_PROP) && startOfPrevWord != -1)
                {
                    startOfPrevWord++;
                    string prevWord = text.Substring(startOfPrevWord, endOfPrevWord - startOfPrevWord).ToLower();

                    Debug.Log("Previous word: \"" + prevWord + "\"");

                    Dictionary<string, string> categoryElements = propertyDescriptors[currentCharacter][wordCategory.Key];
                    if (categoryElements.ContainsKey(prevWord))
                    {
                        newCategoryValue = prevWord;//categoryElements[prevWord];
                    }
                }

                results.Add(wordCategory.Key, newCategoryValue);
            }
        }
    ...
    public void ChangeElements(IDictionary<string, string> changes)
    {
        ...
        foreach (string category in changes.Keys)
        {
            ...
            if (next || newElmName.Equals("prev"))
            {
                for (int i = 0; i < available.Count; i++)
                {
                    if (available[i] != currentConfiguration[category]) continue;
                    if (next)
                        element = i < available.Count - 1 ? available[i + 1] : available[0];
                    else
                        element = i > 0 ? available[i - 1] : available[available.Count - 1];
            ...
            else
            {
                for (int i = 0; i < available.Count; i++)
                {
                    if (available[i].name != newElmName) continue;
                    element = available[i];
                    break;
                }
                if (element == null) throw new Exception("CharacterGenerator.ChangeElements: Element for " + category + " not found: " + newElmName);
            }
    ...
    public void ChangeElements(string configString)
    {
        ChangeElements(ParseForProperties(configString));
    }
}